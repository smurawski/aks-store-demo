...
$SignInName = "$(az account show --query 'user.name' -o tsv)"

az role assignment create --assignee $SignInName --role 'Azure Kubernetes Service RBAC Cluster Admin' --scope $env:AZURE_AKS_CLUSTER_ID
